...
async def testCorrectUserSignin(client=httpx.AsyncClient(base_url=address)) -> None:
    ...
    response = await client.post(url="/api/v1/user/auth/signin", data=correct, headers=headers)
    assert response.status_code == 200
    ...
    response = await client.post(url="/api/v1/user/auth/signin", data=incorrectMail, headers=headers)
    assert response.status_code == 404
    assert response.json() == testResponse

    for i in range(5):
        response = await client.post(url="/api/v1/user/auth/signin", data=incorrectPassword, headers=headers)
...
async def testTokenRefresh(client=httpx.AsyncClient(base_url=address)) -> None:
    ...
    response = await client.get(url="/api/v1/user/auth/refresh", headers=headers)
    assert response.status_code == 200
    print(response.json())
...
async def testMultipleSession(client=httpx.AsyncClient(base_url=address)) -> None:
    ...
    response = await client.post(url="/api/v1/user/auth/logout", headers=headers)
    assert response.status_code == 200
    assert response.json() == testResponse
    ...
    response = await client.get(url="/api/v1/user/auth/session", headers=headers)
    assert response.status_code == 200
    response = response.json()
    ...
    response = await client.post(url="/api/v1/user/auth/logout-session", headers=headers, json=payload)
    assert response.status_code == 200
    ...
    response = await client.post(url="/api/v1/user/auth/logout-all", headers=headers)
...
async def testDeleteUser(client=httpx.AsyncClient(base_url=address)) -> None:
    ...
    response = await client.delete(url="/api/v1/user", headers=headers)
    assert response.status_code == 200
    assert response.json() == testResponse
